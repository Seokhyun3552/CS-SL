...
class MainActivity :
    ...
    SearchView.OnQueryTextListener, MapEventsReceiver {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mapView.setTileSource(
            XYTileSource(
                "CS124", 12, 18, 256, ".png", arrayOf("https://tiles.cs124.org/tiles/")
            )
        )
        ...
        mapView.controller.setCenter(GeoPoint(40.10986682167534, -88.22831928981661))
        val goTofavoritePlace = Intent(this,FavoritePlace::class.java)
        val btn: Button = findViewById(R.id.button_fav)
        for (i in allPlaces.indices) {
            goTofavoritePlace.putExtra("DES" , allPlaces[i].description)
        }
            goTofavoritePlace.putExtra("SIZE", allPlaces.size)
        btn.setOnClickListener {

            startActivity(goTofavoritePlace)
        }
 ...
 */
    ...
    override fun onQueryTextChange(text: String): Boolean {
        ...
        val toReturn = allPlaces.search(text)
        if (toReturn.isEmpty()) {
            updateShownPlaces((allPlaces))
        } else {
            updateShownPlaces(allPlaces.search(text))
        }
        return true
    }

    override fun onQueryTextSubmit(text: String): Boolean {
        val results = allPlaces.search(text)
        if (results.isEmpty()) {
            updateShownPlaces(allPlaces)
        } else {
            updateShownPlaces(allPlaces.search(text))
    ...
    override fun longPressHelper(p: GeoPoint): Boolean {
        Log.d(TAG, "longPress")
        val launch = Intent(this, AddPlaceActivity::class.java)
        launch.putExtra("latitude", p.latitude.toString())
        launch.putExtra("longitude", p.longitude.toString())
